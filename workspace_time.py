...
get_output = lambda x: Popen(x, stdout=PIPE).communicate()[0]
...
class WorkspaceTime:
	...
	def get_display_text(self):
		txt = '\n' + self.get_pretty_time(0) + '  |  ' + self.get_pretty_time(1) + \
			  '\n' + self.get_pretty_time(2) + '  |  ' + self.get_pretty_time(3) + \
			  '\n'
		return txt
	...
	def update_current_position(self):
		...
		viewport = get_output(('xprop', '-root', '-notype', '_NET_DESKTOP_VIEWPORT'))
	...
	def update_current_viewport(self):
		...
		self.current_viewport = self.x / self.width + 2 * (self.y / self.height)
