...
class EngineNotRunningTestCase(EngineBaseTestCase):
    def runTest(self):
        self.assertEqual(self.engine.is_running, False, 'The engine should NOT be running')

class EngineRunningTestCase(EngineBaseTestCase):
	def runTest(self):
		self.engine.start()
		self.assertEqual(self.engine.is_running, True, 'The engine should be running')
		self.engine.stop()

class EngineAppendContainerTestCase(EngineNoContainersBaseTestCase):
	def runTest(self):

		self.assertEqual(len(self.engine.containers), 0, 'Should not contain any container in the beginning')

		c = mock_container.MockContainer()
		self.engine.append_container(c)

		self.assertEqual(len(self.engine.containers), 1, 'A container should be added to the engine')
		
		self.assertIs(c, self.engine.containers[0], 'The container in the engine should be the same added')
...
class EngineRemoveContainerTestCase(EngineNoContainersBaseTestCase):
	def runTest(self):		
		...
		self.assertEqual(len(self.engine.containers), 0, 'The container should have been removed')
		
class SetupContainersTestCase(EngineBaseTestCase):
	def runTest(self):
		self.engine.start()
		c = self.engine.containers[0]
		self.assertEqual(c.count_setups, 1, 'The container should have counted a setup execution')
		self.engine.stop()
...
class UpdateContainersTestCase(EngineFastIntervalBaseTestCase):
	def runTest(self):
		...
		c = self.engine.containers[0]
		self.assertTrue(c.count_updates > 1, 'The container should have counted some update executions')
		self.engine.stop()
